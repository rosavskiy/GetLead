...
async def generate_keywords(niche: str) -> List[str]:
    """
    Генерация ключевых слов на основе описания идеального клиента
    
    Args:
        niche: Описание идеального клиента (человеческим языком)
    ...
    try:
        prompt = f"""Ты - эксперт по лидогенерации в Telegram-чатах.

Пользователь описал своего идеального клиента:
"{niche}"

Твоя задача: сгенерировать ключевые слова и фразы, которые такой клиент мог бы написать в чате, когда ИЩЕТ услугу.

ВАЖНО! Ключевые слова должны:
1. Отражать INTENT клиента (он ищет, ему нужно, он хочет заказать)
2. Быть фразами которые реальный человек напишет в чат
3. Включать разные формулировки одной потребности
4. Быть на русском языке

Хорошие примеры для визовых услуг:
- "нужна виза"
- "ищу кто делает визы"
- "помогите оформить визу"
- "сколько стоит виза"
- "кто оформлял визу"
- "посоветуйте визовый центр"

Плохие примеры (НЕ используй):
- "виза" (слишком общее)
- "получил визу" (клиент уже получил)
- "делаю визы" (это конкурент)

Сгенерируй 15-25 ключевых слов/фраз.
Верни ТОЛЬКО список, каждое с новой строки, без нумерации и пояснений."""

        logger.info(f"Generating keywords for: '{niche[:100]}...'")
        
        response = await client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Ты - эксперт по лидогенерации. Генерируй только фразы которые клиент ПИШЕТ когда ИЩЕТ услугу."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
            max_tokens=800
